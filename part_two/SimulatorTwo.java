...
public class SimulatorTwo {
    ...
    public static void main(String[] args) {
        ...
        TripManagerTwo tripManager = new TripManagerTwo(input);
    ...
    public static class TripManagerTwo extends TripManager {

        // Instance variables
        private List<String> taxiNodes;
        private List<String> shopCompanies;

        // Constructor
        public TripManagerTwo(String graphData) {
            super(graphData);
            this.taxiNodes = new ArrayList<>();
            this.shopCompanies = new ArrayList<>();
            this.populateTaxiNodes();
            this.populateShopCompanies();
        }

        // Method to populate taxi nodes
        private void populateTaxiNodes() {
            this.taxiNodes.add("TaxiNode1"); // Example: Add taxi nodes manually
            this.taxiNodes.add("TaxiNode2");
            // Add more taxi nodes if necessary
        }

        // Method to populate shop companies
        private void populateShopCompanies() {
            this.shopCompanies.add("QnQ"); // Example: Add shop companies manually
            this.shopCompanies.add("Shopfite");
            // Add more shop companies if necessary
        }

        // Method to get taxi nodes
        public String[] taxiNodes() {
            return this.taxiNodes.toArray(new String[this.taxiNodes.size()]);
        }

        // Method to get shop companies
        public String[] shopCompanies() {
            return this.shopCompanies.toArray(new String[this.shopCompanies.size()]);
        }

        // Method to get optimal paths from start to end node with specified company restriction
        public PathAndCost[] getOptimalPathsWithCompany(String start, String end, String company) {
            ...
            while (true) {
                ...
                if (!nodeIsReachable(pathOutput)) {
                    break;
                }
                String[] path = extractPath(pathOutput);
                double cost = extractCost(pathOutput);
        ...
        // Method to get optimal paths to shop with specified company restriction
        public PathAndCost[] getOptimalPathsToShopWithCompany(String node, String company) {
            ArrayList<PathAndCost> optimals = new ArrayList<>();
            for (String shopNode : shopNodes()) {
                if (company != null && !company.isEmpty() && !shopCompany(shopNode).equals(company)) {
                    continue;
                }
                PathAndCost[] optimalsToShop = getOptimalPaths(node, shopNode);
                ...
                if (optimals.isEmpty() || optimals.get(0).cost == optimalsToShop[0].cost) {
                    optimals.addAll(Arrays.asList(optimalsToShop));
                }
            }
            return optimals.toArray(new PathAndCost[optimals.size()]);
        }

        // Method to get the company of a shop
        private String shopCompany(String shopNode) {
            // Example implementation: Assume the first character of the shop node represents the company
            return String.valueOf(shopNode.charAt(0));
        }
    }
    ...
    public static class OptimalTripFormatter {
        ...
        public static String format(String clientNode, PathAndCost[] pathsToClient, PathAndCost[] pathsToShop) {
            ...
            if (pathsToClient.length == 0 || pathsToShop.length == 0) {
                appendClientUnhelpable(clientNode, result);
                return result.toString();
            }
    
            appendTaxiResult(pathsToClient, result);
            appendShopResult(pathsToShop, result);
    ...
    public static class TripManager {
        ...
        protected String[][] getPaths(List<PathAndCost> pathAndCosts) {
        ...
        protected String getOptimalPathOutput(String start, String end, String[][] invalidPaths) {
        ...
        protected static String[] extractPath(String pathOutput) {
        ...
        protected static double extractCost(String pathOutput) {
            String regex = "\\d+\\.\\d+";
        ...
        protected static boolean nodeIsReachable(String pathOutput) {
